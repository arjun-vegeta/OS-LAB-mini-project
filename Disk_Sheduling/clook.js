// JavaScript implementation of the C-LOOK algorithm
...
function CLOOK(arr, head, direction) {
    ...
    // Define the sequence of servicing requests based on the initial direction
    if (direction === "right") {
        // Service requests on the right side of the head first
        ...
        // If there are requests on the left side, jump to the leftmost track
        if (left.length > 0) {
        ...
        // If the initial direction is left, service requests on the left side first
        ...
        // If there are requests on the right side, jump to the rightmost track
        if (right.length > 0) {
            seek_count += Math.abs(head - right[right.length - 1]);
...
document.getElementById("clookForm").addEventListener("submit", function(event) {
    ...
    let direction = directionInput; // Take the selected direction

    // Call the CLOOK function with the user input
    CLOOK(requestArray, headPosition, direction);
});
